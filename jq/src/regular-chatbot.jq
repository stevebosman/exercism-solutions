...
def check_phone_number:
  if test(".*\\(\\+\\d{2}\\) \\d{3}-\\d{3}-\\d{3}.*") then
    ...
    # more complicated than needed for the tests,
    # but extracts a number like value from the input.
    "Oops, it seems like I can't reach out to \(match("[+()\\d\\s-]+"; "gl") | .string | gsub("^\\s+|\\s+$"; "") )."
  end
;
...
def nice_to_meet_you:
  "Nice to meet you, \(capture(".*My name is (?<name>[a-z\\-]+).*"; "i") | .name)"
;
